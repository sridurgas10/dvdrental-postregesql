...
select  title,count(rental_id) as total_rental from film as f,inventory as i, rental as r
where f.film_id=i.film_id and i.inventory_id=r.inventory_id group by f.title order by total_rental desc limit 10
...
select first_name,last_name,sum(amount) as rental_money from customer as c,payment as p,rental as r where c.customer_id=p.customer_id 
and r.rental_id=p.rental_id group by c.customer_id  order by rental_money desc limit 5
...
select st.store_id,sum(p.amount) as revenue from payment as p join store as st on  p.staff_id=st.manager_staff_id
join staff as s  on p.staff_id=s.staff_id group by st.store_id
...
select  city,count(customer_id) as live_City from city as c, address as a, customer as cus 
where cus.address_id=a.address_id and c.city_id=a.city_id group by c.city
...
select name,avg(rental_duration)from category as c,film_category as fc,film as f
where f.film_id=fc.film_id and fc.category_id=c.category_id group by c.name
...
select  max(name)  from film as f,language as l,inventory as i,rental as r
where f.language_id=l.language_id and f.film_id=i.film_id group by l.name
...
select  first_name,last_name,max(amount),sum(amount)from payment as p join staff as s on p.staff_id=s.staff_id group by s.staff_id
